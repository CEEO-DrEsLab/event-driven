...
var url = "http://130.64.94.159:5000";
var lastValues = {};
var config = {};
var trigger_list = {};
var currentValues={}
function parse_data(dataFile) { 
	//Start Data Parsing
...
function is_state_change(response, this_trigger, lastVal) {
...
function send_get(sensorName,trig,index,isFirst) {
	/*SEND A REQUEST WITH:*/
	var get_str = "{'status':'";
		get_str += "get";
		get_str += "','sm_type':'";
...
function send_set(full_config,action_arr) {
	...
	for (var k = 0; k < len; k++) {
		...
		set_str += "}";
		makeCorsRequest(url,set_str);
	}
}

function trigger_settings(inputName, setting) {
...
var parseNameTranslations = { // dictionary to convert port names to EV3 readable format
	"A":"outA",
	"B":"outB",
	"C":"outC",
	"D":"outD",
...
function parse_port(portName) {
	return parseNameTranslations[portName];
}

function set_act_info(mode) {
...
function set_act_val(mode, settings) {
	if (mode == "start_fwd") {
